...
fn tint_symbol_54_inner(tint_symbol_43 : vec3u) {
  ...
  */
  if (x_321 == 10u) {
    // Use a 'for' loop instead of 'loop' for better compiler optimization
    for (var i: i32 = -1; i <= 1; i++) {
        for (var j: i32 = -1; j <= 1; j++) {
            // 1. Skip the center cell early
            if (i == 0 && j == 0) { continue; }

            let nx = bitcast<i32>(x_309) + i;
            let ny = bitcast<i32>(x_310) + j;

            // 2. Combine bounds checks into one 'if'. 
            // D3D12 prefers this over nested 'if' blocks.
            if (nx >= 0 && nx < bitcast<i32>(x_306) && ny >= 0 && ny < bitcast<i32>(x_308)) {
                
                let idx = u32(ny) * x_306 + u32(nx);
                let val = tint_symbol_17.inner[idx];

                // 3. Simple condition check
                if (val != 0u && val != 10u) {
                    let tx = bitcast<i32>(x_309) + tint_symbol_15;
                    let ty = bitcast<i32>(x_310);
                    
                    // Final bounds check for writing
                    if (tx >= 0 && tx < bitcast<i32>(x_306)) {
                        let out_idx = u32(ty) * x_306 + u32(tx);
                        
                        // Use a simple atomic-style check or standard write
                        if (tint_symbol_17.inner[out_idx] == 0u && tint_symbol_18.inner[out_idx] == 0u) {
                            tint_symbol_18.inner[out_idx] = val;
                        }
                    }
                    // IMPORTANT: Avoid 'return' inside loops on D3D12 if possible.
                    // Instead, use variables to break out.
                    return; 
                }
            }
        }
